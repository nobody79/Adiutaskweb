          ...
          {features.map((feature, index) => {
            ...
            return (
              <AnimatedSection key={index} delay={index * 0.08}>
                <div
                  className={`group p-6 rounded-2xl border border-gray-100 ${colors.border} hover:shadow-lg hover:-translate-y-1 transition-all duration-300 h-full`}
                >
                  <div
                    className={`w-12 h-12 rounded-xl ${colors.bg} ${colors.text} flex items-center justify-center mb-4 group-hover:scale-110 transition-transform duration-300`}
                  >
                    {feature.icon}
                  </div>
